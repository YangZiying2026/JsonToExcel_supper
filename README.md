...
- **新功能**：可以在表中添加 **“组合”** 或 **“选科”** 列（例如填入“物化生”），程序将优先读取此信息用于分科统计。
- **文件大小限制**：不超过 20MB。

### 2. 操作步骤

1. **上传 JSON**：点击主区域上传你的成绩单 JSON 文件。
2. **(可选) 关联知识库**：在“高级设置”中点击 Excel 图标上传学生名单。
3. **(可选) 切换模式**：在“高级设置”中开启 **“分科统计模式”** 开关，以生成新高考选科报表。
4. **(可选) 添加水印**：在“高级设置”中上传图片作为背景（图片大小不超过 10MB）。
5. **(可选) 命名文件**：在下方输入框填写期望的输出文件名。
6. **生成报表**：点击底部 "立即生成多维分析报表" 按钮。
7. **下载**：处理完成后点击 "立即下载"。
